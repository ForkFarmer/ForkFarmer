...
public class ForkView extends JPanel {
	...
	public static class ForkTableModel extends JFunTableModel<Fork> implements Reorderable {
		...
		public ForkTableModel() {
			...
			addColumn("Balance",	 80, 	Balance.class,	f->f.balance).show().viewRight();
			addColumn("$",			 60, 	Double.class, 	f->f.price).show().editable();
			addColumn("Equity",		 60, 	Balance.class, 	f->f.equity).viewRight();
			addColumn("Netspace",	 80, 	NetSpace.class, f->f.fd.netspace).show();
			...
			addColumn("ETW",		 70, 	TimeU.class,    f->f.fd.etw).show();
			addColumn("24H Win",	 60,	Double.class, 	f->f.dayWin);
			addColumn("24H BW",		 60,	Long.class, 	f->f.dayWinBC);
			addColumn("Last Win",	 90, 	TimeU.class, 		Fork::getPreviousWin);
			addColumn("Effort",		 50,	Percentage.class, 	Fork::getEffort);
			addColumn("Address",	 450,	Wallet.class, 	f->f.wallet).show().flex();
			addColumn("Reward",		 40,	Double.class, 	f->f.fullReward).editable().colName("RW");
			addColumn("# Wallets",	 40,	Integer.class, 	f->f.walletList.size()).colName("#W");
			addColumn("# Harvesters",40,	Integer.class, 	f->f.numH).colName("#H");
			addColumn("Load", 		 40, 	String.class, 	f->f.load);
			addColumn("Time",		 50,	ReadTime.class, f->f.readTime).show().viewRight();
			addColumn("Full Node",	 30,	Boolean.class, 	f->f.fullNode).editable().fixed().colIco(Ico.P2P_GREY);
